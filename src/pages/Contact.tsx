    ...
    try {
      ...
      if (success) {
        ...
        setError(language === 'en' 
          ...
          : 'செய்தி அனுப்புவதில் தோல்வி. மீண்டும் முயற்சிக்கவும்.'
        );
      }
    } catch (err: any) {
      setError(language === 'en' 
        ? `Error: ${err.message || 'Unknown error occurred'}` 
        : `பிழை: ${err.message || 'அறியப்படாத பிழை ஏற்பட்டது'}`
      );
    } finally {
      setIsLoading(false);
    }
            ...
            <Card className="w-full p-4 sm:p-6 lg:p-8 shadow-elegant">
              <CardHeader className="px-0 pt-0">
                <CardTitle className="font-heading text-lg sm:text-xl lg:text-2xl font-bold text-foreground">
                  {language === 'en' ? 'Send us a Message' : 'எங்களுக்கு ஒரு செய்தி அனுப்புங்கள்'}
                </CardTitle>
              </CardHeader>
                ...
                {isSubmitted ? (
                  ...
                  >
                    ...
                    <h3 className="font-heading text-base sm:text-lg lg:text-xl font-bold text-foreground mb-2">
                      {language === 'en' ? 'Inquiry Sent Successfully!' : 'விசாரணை வெற்றிகரமாக அனுப்பப்பட்டது!'}
                    </h3>
                    <p className="text-muted-foreground text-sm sm:text-base">
                      {language === 'en' 
                        ? 'Thank you for your catering inquiry. We\'ll get back to you within 24 hours with a detailed quote.'
                        : 'உங்கள் கேட்டரிங் விசாரணைக்கு நன்றி. விரிவான மேற்கோளுடன் நாங்கள் 24 மணி நேரத்திற்குள் உங்களைத் தொடர்பு கொள்வோம்.'
                      }
                    </p>
                  </motion.div>
                ) : (
                  <form onSubmit={handleSubmit} className="w-full space-y-4 sm:space-y-6">
                    ...
                    >
                      ...
                      {isLoading 
                        ...
                        : (language === 'en' ? 'Send Inquiry' : 'விசாரணை அனுப்பு')
                      }
                    </Button>
                  </form>
                )}
